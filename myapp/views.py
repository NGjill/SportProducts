...
class detail(ListView):
    ...
    def get_queryset(self):
        cat = Category.objects.get(id=self.kwargs.get('cat_no'))
        list=cat.products.all()
        return list

    def get_context_data(self,**kwargs):
        context = super(detail,self).get_context_data(**kwargs)
        context['category'] = Category.objects.get(id=self.kwargs.get('cat_no'))
        return context
            ...
            if email:
                new_password = "".join(random.sample(
                    ['z', 'y', 'x', 'w', 'v', 'u', 't', 's', 'r', 'q', 'p', 'o', 'n', 'm', 'l', 'k', 'j', 'i', 'h', 'g',
                     'f', 'e', 'd', 'c', 'b', 'a', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0'], 10))
                user.set_password(new_password)
                user.save()
                email_message = "Your new password is " + new_password
                send_mail('new password', email_message, 'u201317544@gmail.com', [email], fail_silently=False)
                message = "Your new password is sent by email"
            else:
                message = "No such user"
    else:
        message = "Your account do not have an email"
    return render(request,'myapp/response.html',{'message':message})


# def client_image_view((request): # 图片上传函数
#     if request.method == 'POST':
#         form = ClientForm(request.POST, request.FILES)
#         form.save()
#     return render(request, 'client_image_view.html')

#pic
def client_image_view(request):
    if request.method == 'POST':
        form = ClientForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            ...
            form = ClientForm()
        return render(request, 'client_image_view.html', {'form': form})
